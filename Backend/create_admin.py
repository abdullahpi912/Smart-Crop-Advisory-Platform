...
def provision_admin():
    ...
    if not favorite_number:
        ...
        if favorite_number != confirm_fn:
            print("Error: Favorite numbers do not match.", file=sys.stderr)
            sys.exit(1)

    if not favorite_number:
        print("Error: Favorite number cannot be empty.", file=sys.stderr)
        sys.exit(1)
    ...
    if not security_phrase:
        ...
        if security_phrase != confirm_sp:
            print("Error: Security phrases do not match.", file=sys.stderr)
            sys.exit(1)

    if not security_phrase:
        print("Error: Security phrase cannot be empty.", file=sys.stderr)
        sys.exit(1)
    ...
    if not security_phrase_hint:
        print("Error: Security phrase hint cannot be empty.", file=sys.stderr)
        sys.exit(1)
